...
public class SiteAction extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SiteAction</title>");
            out.println("</head>");
            out.println("<body>");
            String sitelocation = request.getParameter("sitelocation");
            SiteBean sb = new SiteBean();
            sb.setSitelocation(sitelocation);
            int status= AdminModel.addSite(sb);
            if(status == 1) {
                RequestDispatcher rd = request.getRequestDispatcher("adminPage.jsp");
                rd.include(request, response);
                out.println("Record Inserted");
            }
            out.println("<h1>Servlet SiteAction at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
