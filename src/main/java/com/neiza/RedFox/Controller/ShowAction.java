...
public class ShowAction extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ShowAction</title>");
            out.println("</head>");
            out.println("<body>");
            ...
            sb.setPrice(price);
            int id = new AdminModel().addShow(sb);
            if(id>0) {
                out.println("<b>Show id is"+id+"</b>");
                rd = request.getRequestDispatcher("adminPage.jsp");
                rd.include(request, response);
            }
            out.println("<h1>Servlet ShowAction at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
